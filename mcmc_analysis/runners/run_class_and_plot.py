...
def run_class_and_read_output(param_file, output_prefix, model_label):
    ...
    logging.info(f"Starting CLASS for {model_label}...")
    ...
    try:
        ...
        logging.info(f"CLASS for {model_label} finished successfully.")
        logging.debug(f"CLASS stdout ({model_label}):\n{result.stdout}")
    except FileNotFoundError:
        logging.error(f"Error: Executable '{class_executable}' not found.")
        logging.error("Please ensure CLASS is installed and 'class' is in your system PATH.")
        return None, None
    except subprocess.CalledProcessError as e:
        logging.error(f"CLASS for {model_label} failed with error (return code {e.returncode}):")
    ...
    if not list_of_files:
        logging.error(f"No CLASS output files found for {model_label} with prefix '{output_prefix}'.")
        return None, None
    
    output_file = max(list_of_files, key=os.path.getctime) # Get the latest file by creation time
    
    logging.info(f"Reading results from: {output_file} for {model_label}")
...
def main():
    logging.info("Starting CLASS for PLM-Proxy and ΛCDM-Reference models...")
    ...
    if ll_plm is None:
        logging.error("PLM-Proxy calculations failed. Aborting.")
        return
    ...
    if ll_lcdm_ref is None:
        logging.error("Reference LCDM calculations failed. Aborting.")
        return
    ...
    if not os.path.exists(planck_data_file):
        logging.error(f"Planck data not found in '{planck_data_file}'")
        return
    ...
    plt.plot(ll_plm, dl_tt_plm, color='royalblue', linewidth=2, label='PLM-Proxy Model')
